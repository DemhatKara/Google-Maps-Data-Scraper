...
func NewGmapJob(
	...
	mapURL := ""
	isZeroCoords := geoCoordinates == "0,0" || geoCoordinates == "0.0,0.0" || geoCoordinates == "0, 0"
	if geoCoordinates != "" && zoom > 0 && !isZeroCoords {
...
func WithSearchDelay(d int) GmapJobOptions {
	return func(j *GmapJob) {
		j.SearchDelay = d
	}
}
...
func scroll(ctx context.Context,
	...
	waitTime := 100.
	cnt := 0
	patience := 3 // allow a few "no change" scrolls before giving up
	...
	for i := 0; i < maxDepth; i++ {
		...
		if height == currentScrollHeight {
			...
			// Wait extra time and retry - page may still be loading
			page.WaitForTimeout(1500 * time.Millisecond)
			continue
		}

		patience = 3 // reset after successful scroll
